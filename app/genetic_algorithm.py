...
from app.snake_grid import SnakeGrid
...
class Population:
    ...
    def __init__(self, size: int = 100) -> None:
        self.size = size
        self.agents = [SnakeGrid() for _ in range(self.size)]
        self.best_fitness = 0
    ...
    def crossover(self) -> None:
        ...
        for _ in range(self.size):
            ...
            child = SnakeGrid(chromosome=child_chromosome)
...
def run(generations: int = 10, population_size: int = 100) -> None:
    # initial population
...
if __name__ == '__main__':
    run(generations=1, population_size=1)
